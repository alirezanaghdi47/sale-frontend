...
export const addAdvertiseGallerySchema = Yup.object().shape({
    gallery: Yup.mixed()
        .test("minLength", "حداقل تعداد عکس های ارسالی 1 عدد است", (value: any) => {
            if (value?.length === 0) return false;
            return true;
        })
        .test("maxLength", "حداکثر تعداد عکس های ارسالی 5 عدد است", (value: any) => {
            if (value?.length > 5) return false;
            return true;
        })
        .test("fileSize", "حجم هر عکس باید حداکثر 1 مگابایت باشد", (value: any) => {
        ...
        .test("fileType", "فرمت هر عکس باید از نوع ( png , jpg , jpeg ) باشد", (value: any) => {
                if (value && value.length > 0) {
                    for (let i = 0; i < value.length; i++) {
...
export const EditProfileInformationSchema = Yup.object().shape({
    avatar: Yup.mixed().nullable().test("fileSize", "حجم عکس حداکثر 1 مگابایت باشد", (value: any) => {
        if (value === null) {
            return true;
        } else {
